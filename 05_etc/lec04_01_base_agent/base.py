...
다음 기능을 포함하는 핵심 Agentic Loop 패턴을 구현합니다:
- 도구 호출을 포함한 LLM 상호작용
- 병렬 도구 실행
- 스트리밍 및 논스트리밍 지원
- 상태 관리
    ...
    async def _hook_post_llm_call(self, message: ChatCompletionAssistantMessageParam) -> None:
        ...
        if tool_calls := message.get("tool_calls", []):
            self._current_tool_names = {
                tc["function"]["name"] for tc in tool_calls if tc and tc["function"]["name"]
            }
            tool_messages = await self._execute_tool_calls(list(tool_calls))
            self._extend_messages(tool_messages)
        else:
            self._current_tool_names = set()
