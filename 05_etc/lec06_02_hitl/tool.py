"""HITL 지원이 포함된 에이전트 실행용 도구 인터페이스.

lec04_01_base_agent/tool.py를 확장하여 HITL 기능을 추가합니다.

이 모듈은 도구 추상화에 HITL (Human-In-The-Loop) 기능을 확장합니다.
도구는 이제 사용자 입력을 요청하기 위해 hitl_data가 포함된 ToolResult를 반환할 수 있습니다.
"""
...
class BaseTool(ABC, Generic[TState]):
    """모든 에이전트 도구를 위한 기본 클래스.

    도구는 외부 시스템과 상호작용, 데이터베이스 쿼리, 계산 수행 등을 통해
    에이전트의 기능을 확장합니다. 각 도구는 다음을 정의합니다:
    - LLM이 언제 사용할지 이해하기 위한 이름과 설명
    - 파라미터를 위한 JSON 스키마 (LLM이 유효한 호출을 생성하는 데 사용)
    - ToolResult를 반환하는 실행 로직

    도구 생명주기:
    1. LLM이 이름/설명을 기반으로 도구 호출을 결정
    2. 에이전트가 _pre_execute_hook을 호출하여 상태 의존적 인자 주입
    3. _execute 메서드를 통해 도구 실행
    ...
    strict: bool = True  # 파라미터 검증을 위한 OpenAI strict 모드

    def to_chat_completion_tool(self) -> ChatCompletionToolParam:
        """도구를 LLM 호출용 LiteLLM 형식으로 변환합니다.

        이 메서드는 LLM이 사용 가능한 도구와 호출 방법을 이해하는 데 사용하는
        JSON 구조를 생성합니다.
