"""Cache Control이 포함된 BaseAgent.

확장 원본: lec06_02_hitl/base.py

이 버전의 변경 사항:
- 추가: _hook_pre_llm_call()에서 apply_cache_control_blocks 적용
"""
...
class BaseAgent(ABC, Generic[TState]):
    """Cache Control과 HITL 지원이 포함된 모든 에이전트를 위한 기본 클래스.
    ...
    - Cache Control 최적화 (lec08_04_cost에서 추가)
    ...
    async def _hook_pre_llm_call(self) -> list[ChatCompletionMessageParam]:
        ...
        # [ADDED in lec08_04_cost] 캐시 제어 블록 적용
        return cast(
