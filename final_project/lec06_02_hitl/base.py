"""HITL 지원이 포함된 기본 에이전트 클래스.

lec04_01_base_agent/base.py를 확장하여 HITL 기능을 추가합니다.

이 모듈은 핵심 Agentic Loop에 HITL (Human-In-The-Loop) 기능을 확장합니다:
- 도구 결과에서 HITL 인터럽트 감지
- HITL 인터럽트 존재 시 자동 일시정지
- 사용자 응답 후 실행 재개 메커니즘

주요 HITL 흐름:
1. 도구가 사용자 입력이 필요할 때 ToolResult(hitl_data=HITLData(...))를 반환
2. _execute_tool_calls()가 HITLData를 수집하여 state.hitl_interrupts에 추가
3. hitl_interrupts가 비어있지 않으면 _should_stop()이 True 반환
4. 에이전트 루프가 중단되어 사용자 응답 대기
5. 사용자가 응답하면 _resume_hitl()을 호출하여 계속 실행
"""
...
class BaseAgent(ABC, Generic[TState]):
    """HITL 지원이 포함된 모든 에이전트를 위한 기본 클래스.
    ...
    async def _hook_pre_llm_call(self) -> list[ChatCompletionMessageParam]:
        ...
        Returns:
            list: LLM에 전송할 메시지.
        """
        return copy.deepcopy(self.state.messages)
    ...
    async def _hook_post_llm_call(self, message: ChatCompletionAssistantMessageParam) -> None:
        ...
        if tool_calls := message.get("tool_calls", []):
            tool_messages, hitl_interrupts = await self._execute_tool_calls(
                list(tool_calls)
            )  # [ADDED in lec06_02_hitl: hitl_interrupts 수집]
    ...
    async def _execute_tool_calls(
        ...
        list[ChatCompletionToolMessageParam], list[HITLData]
    ]:  # [ADDED in lec06_02_hitl: hitl_interrupts 반환]
        """여러 도구 호출을 병렬로 실행합니다.
        ...
        # [ADDED in lec06_02_hitl: HITL 인터럽트 확인]
        for tool_call, result in zip(tool_calls, tool_results):
            if getattr(result, "hitl_data", None):
                # LLM 응답의 tool_call_id를 hitl_data에 주입
                result.hitl_data.tool_call_id = tool_call.get("id") or ""  # type: ignore
                hitl_interrupts.append(result.hitl_data)  # type: ignore
            else:
                tool_messages.append(self._build_tool_message(tool_call, result))

        return tool_messages, hitl_interrupts  # [ADDED in lec06_02_hitl: hitl_interrupts 반환]
    ...
    @observe(capture_input=False, capture_output=False)
    async def _resume_hitl(self) -> None:  # [ADDED in lec06_02_hitl]
        ...
        2. mode="tool_result": 사용자가 결과를 직접 제공함 - 도구 메시지로 주입

        Raises:
            Exception: HITL 인터럽트가 없는 경우
        """
